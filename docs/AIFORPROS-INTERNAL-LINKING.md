...
- **Allowed source types (Phase 1 — may be inspected AND edited under supervised-apply):** `cross_reference`. This is the page type the current gap-filling project targets, and where this document is currently strongest.
...
1. **If a genuine, relevant, verifiable (§8) contextual opportunity exists** — a contextual in-body mention SHOULD be proposed and logged for review. It MUST NOT be published without human approval (§15).
2. **If it's unclear whether a genuine opportunity exists** — the pair MUST be routed to human review, not decided by the system.
3. **If no natural opportunity exists** — the table/footer link MUST remain as the only link for that pair, and a human reviewer MUST confirm and record the `no_contextual_opportunity` conclusion for that specific pair. **A model confidence score alone MUST NOT finalize this outcome** — a negative judgment ("there is no good reason to link this") is exactly the kind of decision that needs a human check, not an automated pass. A confirmed `no_contextual_opportunity` conclusion SHOULD expire and be re-evaluated if the source page, target page, this policy, or the supporting evidence record changes.
...
- Batch any automated rollout (§5) — spread writes out; do not push all outstanding gap-fills in one commit.
- Human review before publish is the default (§15).
...
- **Review-required changes:** adding a new contextual link; changing anchor wording; adding or modifying a comparison sentence; removing a link because it appears unnecessary; replacing one link target with a different page; making any claim about product capabilities, pricing, limits, privacy, or suitability.
